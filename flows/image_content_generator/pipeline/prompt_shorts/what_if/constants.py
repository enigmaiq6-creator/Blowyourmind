...
2. **Scenes 2-4 - The Breakdown [10-45s]**: 3-4 scenes each revealing a different consequence. One per scene: geopolitical shift, economic impact, military/power realignment. Each scene must use a world map or infographic visualization.
...
- `visual_type`: Use "ai_image" for all scenes (map-based infographics generated by AI).
- `image_prompt`: MUST follow this structure exactly:
  "Map visualization / infographic style. [THE SPECIFIC VISUAL — describe what the map shows, which countries/regions are highlighted, what colors, what arrows/labels/data points appear]. 9:16 vertical YouTube Shorts frame, clean modern infographic documentary style, dark navy background with subtle grid lines, minimal and uncluttered, crisp sans-serif text in white, professional documentary aesthetic, volumetric soft lighting from top, photorealistic map textures, premium educational style, 4K"
...
2. Every image_prompt MUST include: (a) a map or infographic visualization, (b) the dominant accent color stated explicitly, (c) specific geographic elements (countries, regions, arrows, data labels), (d) the style reference "clean modern infographic documentary style, dark navy background".
3. Each scene must look visually DIFFERENT from the previous one (different map, different data, different visualization).
