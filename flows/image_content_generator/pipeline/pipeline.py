        ...
        if category == "fact_split":
            ...
            for scene in script.scenes:
                ...
                if img_a.exists() and sujeto in ("A", "ambos"):
                    ...
                    filters.append(f"[bg][img_a]overlay=x=40:y=220[bg]")
                    idx += 1
                    # Label above subject A
                    _name_a = sujeto_a.replace("'", "'\\\\''").replace(":", "\\:").replace("%", "\\%")
                    filters.append(f"[bg]drawtext=text='{_name_a}':fontsize=32:fontcolor=white:box=1:boxcolor=black@0.6:boxborderw=8:x=280-text_w/2:y=195:fontfile='C\\:\\\\Windows\\\\Fonts\\\\arial.ttf':borderw=2:bordercolor=black@0.8[bg]")
                ...
                if img_b.exists() and sujeto in ("B", "ambos"):
                    ...
                    filters.append(f"[bg][img_b]overlay=x=560:y=220[bg]")
                    idx += 1
                    # Label above subject B
                    _name_b = sujeto_b.replace("'", "'\\\\''").replace(":", "\\:").replace("%", "\\%")
                    filters.append(f"[bg]drawtext=text='{_name_b}':fontsize=32:fontcolor=white:box=1:boxcolor=black@0.6:boxborderw=8:x=800-text_w/2:y=195:fontfile='C\\:\\\\Windows\\\\Fonts\\\\arial.ttf':borderw=2:bordercolor=black@0.8[bg]")
                ...
                if stickman_timeline:
                    for tl_item in stickman_timeline:
                        ...
                        filters.append(f"[bg][sm{idx}]overlay=x=215:y=820{enable}[bg]")
                        idx += 1
                elif stickman.exists():
                    inputs.append(f"-i {stickman}")
                    filters.append(f"[{idx}:v]scale=650:975,format=rgba[sm]")
                    filters.append(f"[bg][sm]overlay=x=215:y=820[bg]")
                    idx += 1
